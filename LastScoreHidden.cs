...
public class LastScoreHidden : MonoBehaviour
{
    ...
    {
        scores = 0;
        scores = GameController.ScH(); //GameControllerからmainのスコアを取得 scoresはStart()トークン内で定義されているため他の関数では使えない
    ...
    void Update()
    {
        ...
        if (PlayerPrefs.GetInt("HighScoreHidden") < bonusScore)
        {
            PlayerPrefs.SetInt("HighScoreHidden", bonusScore);
        }
